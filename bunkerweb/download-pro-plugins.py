...
try:
    ...
    if pro_license_key and not force_update:
        ...
        while retry_count < max_retries:
            try:
                resp = get(f"{API_ENDPOINT}/pro/status", headers=headers, json=data, timeout=30, allow_redirects=True)
                break
    ...
    if metadata["is_pro"]:
        ...
        while retry_count < max_retries:
            try:
                resp = get(f"{API_ENDPOINT}/pro/download", headers=headers, json=data, timeout=30, stream=True, allow_redirects=True)
                break
            except ConnectionError as e:
                retry_count += 1
                if retry_count == max_retries:
    ...
    if not metadata["is_pro"]:
        ...
        while retry_count < max_retries:
            try:
                resp = get(f"{PREVIEW_ENDPOINT}/v{data['version']}.zip", timeout=30, stream=True, allow_redirects=True)
                break
            except ConnectionError as e:
                retry_count += 1
                if retry_count == max_retries:
